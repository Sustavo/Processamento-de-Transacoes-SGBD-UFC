...
func RL(trManager *[]*TrManagerItem, lockTable *[]*LockTableItem, waitFor *[]*WaitForItem, grafoEspera *[]Tupla, operacao *LockTableItem) int {
	...
	for _, transacao := range *trManager {
		if transacao.trID == operacao.trID && transacao.status == 0 {
			...
			fmt.Println(fmt.Sprintf("T%d obteve RL(%s)", operacao.trID, operacao.idItem))
...
func WL(trManager *[]*TrManagerItem, lockTable *[]*LockTableItem, waitFor *[]*WaitForItem, grafoEspera *[]Tupla, operacao *LockTableItem) int {
	...
	for _, transacao := range *trManager {
		if transacao.trID == operacao.trID && transacao.status == 0 {
			...
			fmt.Println(fmt.Sprintf("T%d obteve WL(%s)", operacao.trID, operacao.idItem))
		...
		if idItem != "" {
			if bloqueio.idItem == idItem && bloqueio.trID == trID {
				...
				*lockTable = append((*lockTable)[:idx_bloqueio], (*lockTable)[idx_bloqueio+1:]...)
				...
				fmt.Println(fmt.Sprintf("T%d liberou %sL(%s)", trID, tipo_bloqueio, idItem))
			}
...
func WAIT(trManager *[]*TrManagerItem, grafoEspera *[]Tupla, waitFor *[]*WaitForItem, operacao *LockTableItem, transacao_detentora int) Tupla {
	...
	if operacao.trID > transacao_detentora {
		...
		fmt.Println(fmt.Sprintf("T%d Aborta (Wait die) ", operacao.trID))
		return tupla_padrao
	}
	...
	for _, tupla := range *grafoEspera {
		if tupla.p1 == operacao.trID && tupla.p2 == transacao_detentora {
			...
			fmt.Println(fmt.Sprintf("T%d gerou Deadlock (T%d-T%d)", tupla.p1, tupla.p1, tupla.p2))
			return tupla
		}
	}
	...
	*grafoEspera = append(*grafoEspera, nova_tupla)
	...
	fmt.Println(fmt.Sprintf("OP%d entrou na Fila (Item %s, Transação %d)", operacao.trID, operacao.idItem, transacao_detentora))
...
func main() {
	...
	fmt.Scanln(&str);
	fmt.Println("Escolha um nível de isolamento: \n 0 - Rean Uncommitted \n 1 - Read Committed \n 2 - Repeatable Read \n 3 - Serializable");
	...
	for _, operacao := range partes {
		...
		if string(operacao[0]) == "B" {
			...
			fmt.Println();
			fmt.Println(fmt.Sprintf("T%d iniciou", trID))
			...
			for _, transacao := range trManager {
				...
				if transacao.trID == trID && transacao.status != 2 {
					...
					fmt.Println(fmt.Sprintf("T%d solicitou WL(%s)", trID, idItem))
					res_WL := WL(&trManager, &lockTable, &waitFor, &grafoEspera, &operacao)
			...
			for _, transacao := range trManager {
				...
				if transacao.trID == trID && transacao.status != 2 {
					...
					fmt.Println(fmt.Sprintf("T%d solicitou RL(%s)", trID, idItem))
					res_RL := RL(&trManager, &lockTable, &waitFor, &grafoEspera, &operacao)
			...
			for _, transacao := range trManager {
				...
				if transacao.trID == trID && transacao.status != 2 {
					...
					fmt.Println(fmt.Sprintf("T%d solicitou C%d", trID, trID))
		...
		fmt.Println("-------- Tabela Lock Table -------");

		fmt.Println("----------------------------------")
		fmt.Println(" ITEM   ID    ESCOPO   DURA  TIP0 ")
		...
		fmt.Println("----- Tabela Wait For Table ------")
		fmt.Println("----------------------------------")
		fmt.Println("          ID    OPERACAO          ")
		/* indice := 0; */
		if (len(waitFor) >= 1) {
		...
		fmt.Println("----- Tabela Grafo de Espera -----");
		fmt.Println("----------------------------------")
